...
class OFold8F3BestEmitter:
    ...
    def __init__(self, NH=8, E=2048, H=8192, G=32, M=4, HD=64, AG=4, SEQ=256, POS=5):
        ...
        # ── per-tile weight tile counts (FULL-K, backport HEAD 02/09) ──
        ...
        # 1B (E=2048,H=8192): 64/256/256 == era (inchangé) ; 9B (E=4096,H=12288):
        # 256/768/768 == 2x era (128/384/384) — HALF-K corrigé (8→16 chunks).
        _N = 32; _KC = 256
        self.Q_T = (self.PER_TILE // _N) * (self.E // _KC)  # Q/O weight tiles per tile (1B: 64, 9B: 256)
        self.GEMV_T = self.Q_T
        self.OPROJ_T = self.Q_T
        self.GU_T = (self.H8 // _N) * (self.E // _KC)        # gate/up (1B: 256, 9B: 768)
        ...
        self.GU_T //= _FFN_DIV
        self.DN_T = (self.E // _N) * (self.H8 // _KC)        # down (1B: 256, 9B: 768)
        self._ffn_div = _FFN_DIV
        # AM-103 (02/09): FULL-K chunk counts passed as nchunk to the GEMV wrappers.
        # era hardcoded nchunk=8 (Q/O/GU) / 4 (down) = HALF-K for E=4096/H8=1536.
        self.NC_Q  = self.E // _KC     # Q/O/GU activation chunks (1B: 8, 9B: 16)
        self.NC_DN = self.H8 // _KC    # down activation chunks (1B: 4, 9B: 6)

        self.WT_TILES = self.GEMV_T + self.OPROJ_T + 2 * self.GU_T + self.DN_T  # (1B no-KV: 896)
        self.WT_BYTES = self.WT_TILES * self.PACKED

        # AM-102 (02/09): qwen35 full-attn per-head sigmoid gate (F3BEST_GATE_FULLATTN=1).
        # The attn-gate GEMV consumes its own A-stream section between Q and O with
        # FULL-K coverage (blocks = PER_TILE/32, chunks = E/256) so the gate vector
        # is computed from ALL of x (unlike the era's half-K Q/O/G/U/D loops).
        self.attn_gate = _os.environ.get('F3BEST_GATE_FULLATTN', '').strip() != ''
        self.AGT_T = 0
        if self.attn_gate:
            self.AGT_T = (self.PER_TILE // 32) * (self.E // 256)   # 9B: 16*16=256 (2x GEMV_T)
        self.O_TILES = E // M
        self.WO_BYTES = self.O_TILES * self.PACKED         # unused arg3 placeholder

        # ── buffer sizes (bf16 elements unless noted) ──
        # #211: the rope LUT region in the x_bundle holds AG*HD angle entries
        # (all Q heads for one KV head). The old fixed "+256" only fit the 1B
        # LUT (AG=4×HD=64). Qwen3.5 (AG=2, HD=256) needs AG*HD=512 → widen XB
        # to E + max(256, AG*HD) + 16 so the seq-meta slot (E+AG*HD) is not
        # overwritten by the LUT.
        self.XB = E + max(256, self.AG * HD) + 16          # x_bundle (1B: 2320, Qwen: 4624)
        self.QI = self.PER_TILE                            # quadrant size (1B: 256)
        # #211: Q_SZ must hold AG*HD Q rows (each score tile processes all
        # attn_group Q heads for its KV head) + the RoPE angle LUT (HD) + pad.
        # PER_TILE (=E/NH) only equals AG*HD when num_q==NH*AG (1B: 256==4*64).
        # Qwen3.5 (num_q=16, NH=8 → AG=2) still has AG*HD=512==PER_TILE ✓, but
        # models where num_q>NH*AG (e.g. the raw 32 Q heads of qwen35 wq) need
        # the AG*HD buffer. Use max() to stay correct for both.
        self.Q_SZ = self.AG * HD + HD + 2                  # Q buffer (1B: 322, Qwen: 770)
        ...
        # ── attention chunking ──
        # #211: CHUNK is NOT fixed at 128 — the score tile K buffer is
        # CHUNK*HD bf16 and must stay well inside the 64KB AIE L1. The 1B
        # geometry (HD=64) fits 128-token chunks (KCH=8192), but Qwen3.5-9B
        # (HD=256) would need KCH=32768 bf16 = 64KB alone — allocation fails.
        # Scale CHUNK so KCH <= 16384 bf16 (32KB) regardless of head_dim,
        # i.e. CHUNK = min(128, 16384 // HD). 128 for 1B (unchanged), 64 for
        # HD=256 (Qwen3.5), and stays power-of-two for the chunk loop.
        self.CHUNK = max(32, min(128, 16384 // HD))
    ...
    def _read_env_flags(self):
        ...
        # hand-asm bcast path (#188 fix: kc256.s cml-init corrected; RR = kc256_rr.s)
    ...
    def _compute_l1_budget(self):
        ...
        agt_bytes = (self.PER_TILE * 2) if self.attn_gate else 0   # AM-102 gate buffer

        self._single_l1 = (a_bytes + b_bytes + q_bytes + o_bytes + p_bytes
                          + g_bytes + u_bytes + s_bytes + uni_bytes + agt_bytes)
    ...
    def _center_triple_b(self, h, p):
        ...
        # AM-102: qwen35 per-head attn gate (conditional emission)
        _agt_buf = _agt_consts = _agt_loop = _agt_mul = ""
        if self.attn_gate:
            _agt_buf = f'    %{p}_agt = aie.buffer(%t{h}) {{sym_name = "{p}_agt"}} : memref<{PT}xbf16>\n'
            _agt_consts = (f'      %c16 = arith.constant {self.NC_Q} : i32\n'
                           f'      %cAG = arith.constant {self.AGT_T} : index\n'
                           f'      %cPt = arith.constant {PT} : i32\n')
            _agt_loop = f'''        // ---- phase 1.5: qwen35 per-head attn gate GEMV (B0 = x_bundle, FULL-K nchunk=16) ----
        scf.for %j = %c0 to %cAG step %c2 {{
          aie.use_lock(%{p}_A0c, AcquireGreaterEqual, 1)
          %ji0 = arith.index_cast %j : index to i32
          func.call @generic_bcast_gemv_bf16_ag(%ji0, %{p}_A0, %{p}_B0, %{p}_uni_partial, %c16, %{p}_agt) : (i32, memref<{PK}xi8>, memref<{XB}xbf16>, memref<{UNI}xi8>, i32, memref<{PT}xbf16>) -> ()
          ...
          func.call @generic_bcast_gemv_bf16_ag(%ji1, %{p}_A1, %{p}_B0, %{p}_uni_partial, %c16, %{p}_agt) : (i32, memref<{PK}xi8>, memref<{XB}xbf16>, memref<{UNI}xi8>, i32, memref<{PT}xbf16>) -> ()
          aie.use_lock(%{p}_A1p, Release, 1)
        }}
'''
            _agt_mul = f'''        // AM-102: qwen35 per-head sigmoid gate on attn_out (B1[0..PT))
        func.call @layer_fused_attn_gate_bf16(%{p}_agt, %{p}_B1, %cPt) : (memref<{PT}xbf16>, memref<{XB}xbf16>, i32) -> ()
'''
    ...
    %core{h} = aie.core(%t{h}) {{
      ...
      %c4 = arith.constant {self.NC_DN} : i32
      %c8 = arith.constant {self.NC_Q} : i32
      {_agt_consts}
    ...
    def _center_single_b(self, h, p):
        ...
        if self.attn_gate:
            ...
            _agt_consts = (f'      %c16 = arith.constant {self.NC_Q} : i32\n'
                           f'      %cAG = arith.constant {self.AGT_T} : index\n'
                           f'      %cPt = arith.constant {PT} : i32\n')
            _agt_loop = f'''        // ---- phase 1.5: qwen35 per-head attn gate GEMV (B = x_bundle, FULL-K nchunk=16) ----
        scf.for %j = %c0 to %cAG step %c2 {{
          aie.use_lock(%{p}_A0c, AcquireGreaterEqual, 1)
          %ji0 = arith.index_cast %j : index to i32
          func.call @generic_bcast_gemv_bf16_ag(%ji0, %{p}_A0, %{p}_B, %{p}_uni_partial, %c16, %{p}_agt) : (i32, memref<{PK}xi8>, memref<{XB}xbf16>, memref<{UNI}xi8>, i32, memref<{PT}xbf16>) -> ()
          aie.use_lock(%{p}_A0p, Release, 1)
          aie.use_lock(%{p}_A1c, AcquireGreaterEqual, 1)
          %j1 = arith.addi %j, %c1 : index
          %ji1 = arith.index_cast %j1 : index to i32
          func.call @generic_bcast_gemv_bf16_ag(%ji1, %{p}_A1, %{p}_B, %{p}_uni_partial, %c16, %{p}_agt) : (i32, memref<{PK}xi8>, memref<{XB}xbf16>, memref<{UNI}xi8>, i32, memref<{PT}xbf16>) -> ()
          aie.use_lock(%{p}_A1p, Release, 1)
        }}
'''
            _agt_mul = "" if _os.environ.get('F3BEST_GATE_NOMUL', '').strip() != '' else f'''        // AM-102: qwen35 per-head sigmoid gate on attn_out (B[0..PT))
        func.call @layer_fused_attn_gate_bf16(%{p}_agt, %{p}_B, %cPt) : (memref<{PT}xbf16>, memref<{XB}xbf16>, i32) -> ()
'''
    ...
    def emit_mlir(self):
        ...
        funcs = (
            ...
            + (f'    func.func private @layer_fused_dump_q_bf16(memref<{QSZ}xbf16>, memref<{PT}xbf16>, i32) attributes {{link_with = "layer_fused_relay.o"}}\n'
               if self.QDUMP else '')
            + (f'    func.func private @generic_bcast_gemv_bf16_ag(i32, memref<{PK}xi8>, memref<{XB}xbf16>, memref<{UNI}xi8>, i32, memref<{PT}xbf16>) attributes {{link_with = "layer_fused_unified_bcast.o"}}\n'
               f'    func.func private @layer_fused_attn_gate_bf16(memref<{PT}xbf16>, memref<{XB}xbf16>, i32) attributes {{link_with = "layer_fused_relay.o"}}\n'
               if self.attn_gate else '')
        )
